...
type MethodsFromABI<T extends ABI> = {
  [K in T[number] as K['name']]: MethodFromEntry<K>
};
...
export class TypedContract<T extends ABI> {
    ...
    return new Proxy(this, {
      get(target, prop, receiver) {
        ...
        if (typeof prop === 'string' && target.methods.has(prop)) {
          return (params: any) => target.invokeMethod(prop, params);
        }
   ...
   */
  private invokeMethod(methodName: string, params: any = {}): Record<string, any> {
   ...
   */
  public invoke<K extends T[number]['name']>(
    methodName: K,
    params: K extends T[number]['name'] 
