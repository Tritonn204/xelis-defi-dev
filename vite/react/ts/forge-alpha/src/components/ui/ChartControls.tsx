...
import { useChart, type ChartDisplayMode } from '@/contexts/ChartContext';
import { RES_LABEL, UI_RES_OPTIONS } from '@/types/chart';
...
type Props = {
  className?: string;
  onJumpToRecent?: () => void; // Add callback prop
};

export const ChartControls: React.FC<Props> = ({ className, onJumpToRecent }) => {
  const {
    resolution,
    setResolution,
    displayMode,
  ...
  const pairRef = useClickOutside<HTMLDivElement>(pairOpen, () => setPairOpen(false));
  const resRef = useClickOutside<HTMLDivElement>(resOpen, () => setResOpen(false));
  ...
  const displayModeOptions: { key: ChartDisplayMode; label: string }[] = React.useMemo(() => {
    if (!tokenPair) return [{ key: 'base', label: tickerSymbol }];

    return [
      { key: 'base', label: `${tokenPair.base.ticker} - ${tokenPair.quote.ticker}` },
      { key: 'inverted', label: `${tokenPair.quote.ticker} - ${tokenPair.base.ticker}` },
      { key: 'base-usd', label: `${tokenPair.base.ticker} - USD` },
      { key: 'quote-usd', label: `${tokenPair.quote.ticker} - USD` },
    ];
  }, [tokenPair, tickerSymbol]);

  const currentDisplayLabel = displayModeOptions.find(o => o.key === displayMode)?.label || tickerSymbol;
  const currentResLabel = UI_RES_OPTIONS.find(o => o.value === resolution)?.label || resolution;

  return (
    <div className={className}>
      {/* Top row - Pair selector and Resolution */}
      <div className="flex items-center gap-3 mb-2 ml-1">
        {/* Pair/Display Mode Dropdown */}
        <div ref={pairRef} className="relative">
          ...
          {pairOpen && (
            <div
              role="menu"
              className="absolute left-0 top-full mt-1 w-48 rounded-md border border-white/10 bg-[#0e121a]/95 shadow-lg p-1 text-sm z-50"
            >
              {displayModeOptions.map(item => {
                const selected = displayMode === item.key;
                return (
                  <div
                    key={item.key}
                    role="menuitemradio"
        ...
        <div ref={resRef} className="relative">
          <div
            ...
            className="
              text-neutral-300 text-xs cursor-pointer
              flex items-center gap-1
          ...
          {resOpen && (
            <div
              role="menu"
              className="absolute left-0 top-full mt-1 w-16 rounded-md border border-white/10 bg-[#0e121a]/95 shadow-lg p-1 text-xs z-50"
            >
              {UI_RES_OPTIONS.map(item => {
                const selected = resolution === item.value;
                return (
                  <div
                    key={item.value}
                    role="menuitemradio"
                    ...
                    className={`
                      w-full text-center px-2 py-1 rounded cursor-pointer
                      hover:bg-white/5 
                      ${selected ? 'text-white bg-white/5' : 'text-gray-300'}
                      transition-colors
                    `}
      ...
      <div className="flex items-center gap-1">
            ...
            >
                ...
                <div className="flex gap-2">
                  {[9, 20, 50].map(p => (
                    <Button
                      ...
                      className="px-2 py-1 text-xs rounded-full bg-white/5 hover:bg-white/10 text-neutral-200 border border-white/10"
                    >
                      EMA {p}
                    </Button>
                  ))}
              ...
              <div className="space-y-2 max-h-56 overflow-auto pr-1">
                {indicators.ema.map(e => (
                  <div
                    key={e.id}
                    className="flex items-center gap-2 rounded-full backdrop-blur-sm bg-white/5 border border-white/10 px-3 py-1"
                  >
                    <input
                      type="checkbox"
                      checked={e.enabled}
                    ...
                    <input
                      type="number"
                      className="w-16 bg-[#0e121a] text-neutral-200 border border-white/10 rounded-full px-2 py-1 text-xs"
                      min={1}
                      value={e.period}
                      onChange={(ev) => {
                    ...
                    <input
                      type="color"
                      aria-label={`EMA ${e.id} color`}
                      className="h-6 w-6 cursor-pointer p-0.5 bg-[#0e121a] appearance-none rounded-full border-2 border-[#0e121a] hover:p-0.25 transition-all"
        ...
        <div ref={candleRef} className="relative">
          <Button
        ...
        {onJumpToRecent && (
          <Button
            ...
            className="
              rounded-md bg-black/40 hover:bg-black/60 border border-white/10 
              px-2 py-1 text-xs text-neutral-200 hover:text-white
              transition-all duration-175 flex items-center gap-1
            "
